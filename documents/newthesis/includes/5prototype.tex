...
\begin{itemize}
	...
	\item \textbf{\textrm{User Request Handler}:} The user interface modules to administrate \textrm{Event Triggers}, \textrm{Webhooks}, \textrm{Rules} and \textrm{Action Dispatchers}.
\end{itemize}

When started, the prototype system loads persisted \textrm{Webhooks} and begins to listen for new events on them.
...
The prototype is now up and running and accepts administration requests for \textrm{Event Triggers}, \textrm{Webhooks}, \textrm{Rules} and \textrm{Action Dispatchers}.
...
\begin{itemize}
	...
	\item \textrm{pushEvent}: This is the most important part for \textrm{Event Trigger} modules, since events are only through this function pushed into the prototype system. For \textrm{Action Dispatcher} modules this provides the possibility for loopback events.
\end{itemize}float=h,
By using the power of \textrm{JavaScript} and some of its libraries, it is possible to access a large part of the existing \textrm{\glspl{webresource}} and transform changes in their \textrm{\glspl{infospace}} into events and also to impose changes onto them as part of actions.
...
% TODO automatic detection of required arguments 
...
Only expert users are expected to store such a piece of code float=h,in our prototype, which enables inexperienced users to simply choose the \texttt{"EmailYal -> newMail"} \textrm{Event Trigger} for their rule.  
...
As powerful as \textrm{Webhooks} are in providding real-time notifications from remote \textrm{\glspl{webresource}}, as simple they are to use.
In our prototype, users can create as many new \textrm{Webhooks} as they choose to.
They only need to provide an event name which will be associated to the \textrm{Webhooks}.
...
Any \textrm{\gls{webresource}} that supports the concept of \textrm{Webhooks} (e.g. \textrm{GitHub}\footnote{https://developer.github.com/webhooks/}) has a place to register the \textrm{Webhook} \textrm{\acrshort{uri}}.
...
Since \textrm{Webhhooks} are such an important institution in our vision we also implemented an \textrm{Action Dispatcher} that pushes events to other \textrm{Webhooks}.
...
Tree node selectors for event parameters are used in conditions to select a paremeter which is evaluated.
...
\begin{lstlisting}[language=OwnRule,caption=Extended Backus-Naur Form of Prototype Rule Language Syntax]
...
% Real-time reactive Web over Webhooks -> instant messaging
...
% ode.js is a powerful tool to build cloud applications, which use the in-
% ternet as a communication layer. Because of node.js’s modularity and the
% package manager, slim instances can be run. Its main purpose is to be
% used as a webserver. By incorporating JavaScript’s asynchronous design
...
% and traffic. A name and a version are mandatory for a node.js module.
% Knowing the name, e.g. diff, one can look up the latest version in the npm
% repository by issuing following command:
% $ npm show diff version
% npm http GET https://registry.npmjs.org/diff
% npm http 200 https://registry.npmjs.org/diff
% 1.0.5
% Now this dependency, together with the version one would like to use,
% can be inserted into the own project description file as shown below. For
% our small use case proof of concept project we only had to define the project
% description file called package.json in the root folder of the project:
% {
% "name": "msc-apps",
...
% }
% }
% Everybody who downloads this project needs to download the dependen-
% cies before using the module. This is done by running following command
% in the root directory of the project:
% $ npm install
% If any dependencies change in the future the same command can be run
% again to update the local project. A simple Hello World example of node.js
% looks like this:
...
% });
% server.listen(8000);
% After executing this by typing (if the above code was stored in a file
% called index.js):
% $ node index.js
% the webserver is accessible on port 8000 (by typing localhost:8000 in the
% browser bar), and responds with ”Hello World”. The code of the ECA rules
% engine implementation in node.js can be found in the appendix.
