...
By combining existing research in these fields we developed a conceptual model, which allows to impose smart reactivity to any \textrm{\gls{infospace}}.
...
Events are initially primitive that occur at a point in time (e.g. a press down mouse button event).
...
So far we defined all the modules to access \textrm{\glspl{infosystem}} over their services, now we are able to define a \textrm{Rule Engine} that orchestrates them in a reactive way.
