...
Just imagine how you will get food without money because the credit card reader can't establish a connection to the bank.
If you still got some cash you'll have to save it, since banks will not work anymore when the Web is gone.
\index{World-Wide Web}
Ever since the term \textrm{World-Wide Web}~\cite{DBLP:journals/en/Berners-LeeCGP92} started to exist there has always been a trend towards services~\cite{peltz2003web} which are offered via the Web.
\index{SOA}
The encapsulation of functionality into services in order to offer them to other applications is called service-oriented architecture (\textrm{SOA})~\cite{perrey2003service}.
\index{Services}
Adopting \textrm{SOA} internally to an application means splitting the application into smaller pieces, which then communiacte via services among each other.
This does not only provide robustness, it also allows the reuse of functionality.
Moreover these services can be offered to other applications and also to the Web, thus allowing users the access to certain functionalty or even the whole application.
All nodes in the Web are standalone entities, which offer services of some sort, be it a webpage, measurements, functionality or whatsoever can be offered through the Web.
This nature of the Web makes itself a service-oriented architecture and everything these entities offer are \textrm{Services in the Web}.
It is for its advantages that \textrm{SOA} has received a great deal of attention and has been widely adopted, also throughout the Web.
This lead to a rapidly increasing number of Web accessible services and their compositions.
An empirical study~\cite{conf/icws/HuangFT12} on a directory they call the \textit{"[...] most active Web APIs and mashups collection"} and data taken from that resource (depicted in Figure \ref{fig:NumOfAPIs}) seem to draw the same picture.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/NumOfAPIs}
  \caption{Number of registered APIs in the ProgrammableWeb directory by date}
  \label{fig:NumOfAPIs}
\end{figure}

A considerable amount of literature~\cite{2007_AlferesR3}~\cite{2005-Bry_etal-XChange.pdf}~\cite{10.1007-11896548_63}~\cite{Elliott:2009:PFR:1596638.1596643}~\cite{2009-Paschke_Boley-RCER.pdf}~\cite{2012-Paschke_etal-ReactionRuleML.pdf} has been published on reactivity.
These studies underline that rectivity can be imposed through Event-Conditon-Action (\textrm{ECA}) rules.
...
In this chapter we will give a quick overview over the different approaches to the offering of services, in order to draw a picture about what \textrm{Services in the Web} are.
We will then present existing rule languages and engines that exploit ECA paradigm.

\section{Services in the Web}
\index{Web Service}
\index{Web API}
As we have already pointed out, an important development within the Web are the increasing number of available services.
...
\index{RPC}
A fairly early adoption of the service concept to computers were the Remote Procedure Calls (\textrm{RPC})~\cite{Birrell:1984:IRP:2080.357392}.
...
Quicker to implement soap if existing aids are installed, because they aid
...
% TODO dienste services Web API belegen mit referenzen wo begriff wie gebraucht wird, Begriffe festnageln, definieren
...
% TODO From read-only to read-write to programmable
...
\section{Event-Condition-Action Paradigm and Complex Event Processing}
\index{ECA}
